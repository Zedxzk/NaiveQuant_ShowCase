...
> `<workspace>`、`<strategy>`、`<date>` 等占位符。指标数值、页面结构和版本号未作改动。

**研究控制台**：回测记录、因子库与策略库、数据浏览器、交易与成交记录、插件页、命令中心、作业、工作区、终端，支持深浅色主题。下图为回测运行详情——左侧是可按库/类型/状态/Sharpe/Alpha 过滤的运行列表，右侧汇总综合评级、收益与风险、以及因子预测的 OOS IC / RankIC / ICIR 面板。
