...
**② 净值曲线**：策略净值、基准（沪深 300）与初始本金三线对照，可切线性/对数纵轴。这次运行总收益 115.12%、年化 7.51%、年化 Alpha 6.09%，明显跑赢基准。

![净值曲线](assets/equity-dark.png)

**③ 全部指标**：完整指标账本。除常规收益风险指标外，**执行成本是分项列出的**——价差成本、佣金、市场冲击、平台费、滑点、规费税费各自独立成行，并注明成本口径（硬费用现金扣除，执行成本通过成交价影响净值，不重复计入），避免回测收益被隐性成本粉饰。
...
> **关于评级**：这次运行的最终评级是 **淘汰 (REJECTED)**，综合得分 0.00/100，原因是「触发底线：最大回撤超过 25.0%」（实际 42.99%）。
> 评分体系设有风险底线，**一票否决优先于收益表现**——总收益 115% 也不能让一条回撤失控的策略进入生产。
> 展示这一点是有意的：能否否掉自己的策略，比能否跑出好看的净值曲线更能说明评估体系是否可用。
...
- **仿射复权（A/B）**：0.21 起统一复权公式为 `adjusted = raw × A + B`。港股不再靠"前收盘比值"猜复权系数，而是按 Futu 官方 `get_rehab` 事件倒序组合出 A/B，现金分红与拆并股可以落在同一条复权链上；与官方 HFQ 抽样对账的相对误差约 5ppm。A 股、美股的单比例数据以 `B=0` 兼容。
...
聚合层不是简单投票：`quantplatform.committee` 负责稳定性校准、分歧度量、决策区间与黏性控制——按 `provider::model` 索引每个 Agent 的稳定性表，缺少当前模型的专属样本时回退到聚合值，并在报告里明确标记回退来源，不冒充专属校准。输出只是目标权重和最新价，下单、dry-run、paper/live、对账和熔断全部沿用平台既有链路。
...
- **L2 事件执行**：A 股逐笔/盘口链路按一档 participation 上限与整手约束计算开仓数量，严格 T+1 退出时再次检查盘口容量、可成交时间窗、成本和实际持仓；数据缺失一律安全拒绝，不做乐观假设。
...
- **依赖红线可执行**：19 条 import-linter 分层契约写进 `pyproject.toml`，由 `lint-imports` 强制执行——`core` 不得向上依赖、`auth` 只依赖 `core`、计算层不得依赖呈现/入口层等。少量兼容 shim 显式列入白名单并注明原因，实现代码不得走这些边。
- **重复代码整理**：纯语言级工具归入 `core/`，带 DuckDB 语义的归入 `data/`，带 broker 语义的留在 `trading/brokers/`。对于语义已经分化的代码，不会强行合并，而是先将差异参数化。
- **兼容边界明确**：大型入口保留稳定 façade，具体职责拆成可独立测试的模块；旧路径通过 re-export 或延迟属性解析指向同一实现对象，但兼容 façade 不允许增加业务分支。该删的旧实现直接删除，不留"看起来还能用"的转发。
- **版本和变更记录**：提交时同步维护版本常量和 `docs/update-log.md`，按 Added / Changed / Fixed / Tests / Notes 记录；0.14.x 及更早已按次版本系列归档。
- **文档也进检查**：`scripts/check_docs.py` 全量校验本地链接与 Markdown anchor、平台与插件版本一致性、文档索引覆盖、设计/归档状态标识、环境变量和关键数量事实。
