...
上述边界不是文档约定，而是 `pyproject.toml` 中的 **19 条 import-linter 契约**，由 `lint-imports` 校验，例如：
...
少量历史兼容 shim 以 `ignore_imports` 显式登记并注明原因（例如 PEP 562 惰性转发），实现代码不允许使用这些边。
...
复权同样是一个容易被"差不多能用"糊弄过去的地方。平台统一采用**仿射复权**：

```
adjusted_price = raw_price × adj_factor(A) + adj_offset(B)
```

港股的 A/B 由官方复权事件按倒序语义累计得到——新事件 `(a, b)` 加入旧链 `(A, B)` 得 `A' = aA`、`B' = bA + B`。这样现金分红（体现在 B）和拆并股（体现在 A）可以组合在同一条链上，不会像单比例方案那样把分红做成伪跳空。A 股和美股的单比例数据以 `B = 0` 兼容读写。

配套的原则是：**因子请求失败的标的拒绝写入**，不能用 `A=1 / B=0` 静默掩盖公司行为；展示价与成本仍用原始价，技术指标与研究口径用后复权价。
